...
export async function POST(req: Request) {
  try {
    const { name, personality, age, avatar } = await req.json();
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      await resend.emails.send({
        ...
        html: `
          <div style="font-family: sans-serif; color: #333;">
            ...
            ${avatar ? `<img src="${avatar}" style="width:80px;height:80px;border-radius:50%;border:3px solid #d32f2f;margin-bottom:10px;" />` : ''}
      ...
      console.log(`[EMAIL SENT] New player joined: ${name}`);
