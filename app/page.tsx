                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="relative">
                    ...
                    <button 
                      ...
                      className="absolute right-3 top-[calc(50%+1px)] -translate-y-1/2 text-red-400 hover:text-red-200"
                    >
                      {showSignUpPassword ? (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-60 overflow-y-auto p-1 custom-scrollbar">
                    {personalities.map(p => (
                      <div 
                        key={p.id} 
                        onClick={() => { audioEngine.playSelect(); setPersonality(p.id); }} 
                        className={`cursor-pointer p-3 rounded-lg transition-all border ${ 
                          personality === p.id 
                            ? 'bg-red-950/40 border-red-500 shadow-[0_0_20px_rgba(220,38,38,0.3)] scale-[1.02]' 
                            : 'bg-white/5 border-white/10 hover:bg-white/10 hover:border-white/20' 
                        }`} 
                      >
                        <div className={`font-semibold mb-1 ${personality === p.id ? 'text-red-400' : 'text-zinc-200'}`}>{p.id}</div>
                        <div className={`text-xs font-medium ${personality === p.id ? 'text-red-200' : 'text-zinc-400'}`}>{p.desc}</div>
                      </div>
                    ))}
                  </div>
                </div>
